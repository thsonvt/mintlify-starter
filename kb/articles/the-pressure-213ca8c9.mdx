---
title: 'The pressure'
description: 'The curl team faces unprecedented pressure due to a surge in AI-assisted security reports, leading to an increased workload and concerns about work-li'
icon: 'newspaper'
author: 'Simon Willison'
authorId: 'simon-willison'
published: '2026-05-26'
sourceUrl: 'https://simonwillison.net/2026/May/26/the-pressure/#atom-everything'
topics: ["ai-security","curl-project","work-life-balance"]
diataxisType: 'explanation'
---

<Info>
**Original**: [Simon Willison](https://simonwillison.net/2026/May/26/the-pressure/#atom-everything) · 26/05/2026
</Info>

## Summary

The curl team faces unprecedented pressure due to a surge in AI-assisted security reports, leading to an increased workload and concerns about work-life balance.

## Key Insights

> "The rate of incoming security reports is 4-5 times higher than it was in 2024."
>
> — Discussing the increase in security issues faced by the curl team.

> "This is a never-before seen or experienced pressure on the curl project and its security team members."
>
> — Describing the current challenges faced by the curl security team.

> "The good news is that curl is a very solid piece of software, so the vulnerabilities people are finding tend not to be of high severity."
>
> — Highlighting the overall quality of the curl software despite the increase in reports.

## Topics

- [ai-security](/kb/topics/ai-security)
- [curl-project](/kb/topics/curl-project)
- [work-life-balance](/kb/topics/work-life-balance)

---

## Full Article

26th May 2026 - Link Blog The pressure (via) Daniel Stenberg on the unprecedented level of pressure the curl team are facing right now thanks to the deluge of (credible) AI-assisted security issues being reported. The rate of incoming security reports is 4-5 times higher than it was in 2024 and double the speed of 2025 -- meaning that on average we now get more than one report per day. The quality is way higher than ever before. The reports are typically very detailed and long. [...] For the first time in my life, my wife voiced concerns about my work hours and my imbalanced work/life situation. I work more than I’ve done before, but the flood keeps coming. [...] This is a never-before seen or experienced pressure on the curl project and its security team members. An avalanche of high priority work that trumps all other things in the project that is primarily mental because we certainly could ignore them all if we wanted, but we feel a responsibility, we have a conscience and we are proud about our work. The good news is that curl is a very solid piece of software, so the vulnerabilities people are finding tend not to be of high severity: What is also a good trend: almost no one finds terrible vulnerabilities. All vulnerabilities found the last few years in curl have all been deemed severity LOW or MEDIUM. I'm not saying there won't be any more HIGH ever, but at least they are rare. The most recent severity high curl CVE was published in October 2023.

---

## Related Articles

<CardGroup cols={1}>
  <Card
    title="A quote from Daniel Stenberg"
    icon="newspaper"
    href="/kb/articles/a-quote-from-daniel-stenberg-e1df2f9a"
  >
    Simon Willison · explanation · 64% similar
  </Card>
  <Card
    title="A quote from Willy Tarreau"
    icon="newspaper"
    href="/kb/articles/a-quote-from-willy-tarreau-4f4fcb91"
  >
    Simon Willison · explanation · 53% similar
  </Card>
  <Card
    title="Anthropic’s Project Glasswing—restricting Claude Mythos to security researchers—sounds necessary to me"
    icon="newspaper"
    href="/kb/articles/anthropics-project-glasswingrestricting-claude-mythos-to-sec-d2496898"
  >
    Simon Willison · explanation · 52% similar
  </Card>
</CardGroup>

---

<Note>
Originally published at [https://simonwillison.net/2026/May/26/the-pressure/#atom-everything](https://simonwillison.net/2026/May/26/the-pressure/#atom-everything).
</Note>
