...
<script dangerouslySetInnerHTML={{ __html: `
  ...
  function initHighlightsPage() {
    console.log('[MyHighlights] initHighlightsPage called');
    const app = document.getElementById('highlights-app');
    if (!app) {
      console.log('[MyHighlights] No app element found');
      return;
    }
    ...
    console.log('[MyHighlights] API_URL:', API_URL);

    // Check auth state
    let checkCount = 0;
    async function checkAuth() {
      checkCount++;
      console.log('[MyHighlights] checkAuth attempt:', checkCount, 'highlightsAuth exists:', !!window.highlightsAuth);

      if (!window.highlightsAuth) {
        if (checkCount > 50) {
          console.error('[MyHighlights] Gave up waiting for highlightsAuth after 50 attempts');
      ...
      console.log('[MyHighlights] isAuthenticated:', isAuthed);

      if (!isAuthed) {
        renderSignIn();
        window.addEventListener('highlights-auth-change', (e) => {
          console.log('[MyHighlights] auth-change event:', e.detail);
    ...
    function renderSignIn() {
      console.log('[MyHighlights] Rendering sign-in UI');
      app.innerHTML = \`
    ...
    async function loadHighlights() {
      console.log('[MyHighlights] loadHighlights called');
      ...
      console.log('[MyHighlights] Got token:', !!token);

      if (!token) {
        console.log('[MyHighlights] No token, showing sign-in');
        renderSignIn();
        return;
      }

      try {
        console.log('[MyHighlights] Fetching highlights from:', API_URL + '/api/highlights');
        const response = await fetch(API_URL + '/api/highlights', {
          headers: { 'Authorization': 'Bearer ' + token }
        });

        console.log('[MyHighlights] Response status:', response.status);

        if (!response.ok) {
          const errorText = await response.text();
          console.error('[MyHighlights] Error response:', errorText);
          throw new Error('Failed to load');
        }

        const data = await response.json();
        console.log('[MyHighlights] Got highlights:', data);
        renderHighlights(data.highlights || []);

      } catch (err) {
        console.error('[MyHighlights] Load error:', err);
        app.innerHTML = '<div style="padding: 40px; text-align: center; color: #dc2626;">Failed to load highlights. Check console for details.</div>';
      }
    }
